...
fn get_spectral_moment(magnitudes: &[f64], moment: i32) -> f64 {
    ...
    let weighted_sum: f64 = magnitudes.iter().enumerate()
...
pub fn fft_aggregated(x: &[f64], agg_type: &str) -> f64 {
    let fft = compute_rfft(x);
    let magnitudes: Vec<f64> = fft.iter().map(|c| (c.re.powi(2) + c.im.powi(2)).sqrt()).collect();
    ...
    match agg_type {
        ...
        "variance" => variance,
        "skew" => {
            if variance < 0.5 { return f64::NAN; }
