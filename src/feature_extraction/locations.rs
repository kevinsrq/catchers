...
pub fn first_location_of_maximum(x: &[f64]) -> f64 {
    ...
    let (idx, _) = x.iter().enumerate().fold((0, f64::NEG_INFINITY), |(max_idx, max_val), (i, &val)| {
        if val > max_val {
            (i, val)
        } else {
            (max_idx, max_val)
...
pub fn last_location_of_maximum(x: &[f64]) -> f64 {
    ...
    let (rev_idx, _) = x.iter().rev().enumerate().fold((0, f64::NEG_INFINITY), |(max_idx, max_val), (i, &val)| {
        if val > max_val {
            (i, val)
        } else {
            (max_idx, max_val)
...
pub fn first_location_of_minimum(x: &[f64]) -> f64 {
    ...
    let (idx, _) = x.iter().enumerate().fold((0, f64::INFINITY), |(min_idx, min_val), (i, &val)| {
        if val < min_val {
            (i, val)
        } else {
            (min_idx, min_val)
...
pub fn last_location_of_minimum(x: &[f64]) -> f64 {
    ...
    let (rev_idx, _) = x.iter().rev().enumerate().fold((0, f64::INFINITY), |(min_idx, min_val), (i, &val)| {
        if val < min_val {
            (i, val)
        } else {
            (min_idx, min_val)
...
pub fn number_peaks(x: &[f64], n: usize) -> f64 {
    if n == 0 { return 0.0; } // Support 0 undefined or no peaks? tsfresh loop range(1, n+1), so n=0 loops 1..1 empty.
    if x.len() < 2 * n + 1 { return 0.0; }
