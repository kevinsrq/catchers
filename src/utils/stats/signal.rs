...
pub fn welch(signal: &[f64], fs: f64, window: &[f64]) -> (Vec<f64>, Vec<f64>) {
    ...
    if step == 0 { // avoid division by zero if nperseg is huge or something wrong
         return (vec![], vec![]); 
    }
    ...
    let scale_factor = if window_energy > 0.0 { 2.0 / (fs * window_energy) } else { 0.0 };
...
pub fn autocorr(a: &[f64]) -> Vec<f64> {
    ...
    for i in 0..n {
        buffer[i] = Complex { re: a[i] - mean_val, im: 0.0 };
    }
